...
public class EstadoDePersonaje {
	...
	public static void dibujarEstadoDePersonaje(Graphics g, int x, int y, PaqueteNpc personajeNPC, BufferedImage miniaturaPersonaje){
		...
		g.setFont(new Font("Tahoma", Font.PLAIN, 10));
		g.drawImage(Recursos.barraSalud, x + 80, y + 26, ANCHOBARRA, ALTOSALUD, null);
/*		g.drawString(String.valueOf(personajeNPC.getSaludTope()) + " / " + String.valueOf(personajeNPC.getSaludTope()), x + 132, y + 37);

		g.drawImage(Recursos.barraEnergia, x + 80, y + 42, ANCHOBARRA, ALTOENERGIA, null);
		g.drawString(String.valueOf(personajeNPC.getEnergiaTope()) + " / " + String.valueOf(personajeNPC.getEnergiaTope()), x + 132, y + 52);

		if(personajeNPC.getExperiencia() == Personaje.getTablaDeNiveles()[personajeNPC.getNivel() + 1]) {
			drawBarra = ANCHOBARRA;
		} else {
			drawBarra = (personajeNPC.getExperiencia() * ANCHOBARRA) / Personaje.getTablaDeNiveles()[personajeNPC.getNivel() + 1];
		}
