...
public class Trueque extends ComandosEscucha {
	...
	public void ejecutar() {
		...
		juego.getCliente().getPaquetePersonaje().getCasta()).newInstance();
			pj = (Personaje) Class.forName("dominio" + "." +
		juego.getCliente().getPaquetePersonaje().getRaza()).getConstructor(
				String.class, Integer.TYPE, Integer.TYPE, Integer.TYPE,
				Integer.TYPE, Integer.TYPE, Casta.class, Integer.TYPE, Integer.TYPE,
				Integer.TYPE).
					newInstance(nombre, salud, energia,
							fuerza, destreza, inteligencia, casta,
		...
		if (id == paqueteComerciar.getId()) {
			...
			ArrayList<Item> items = juego.getPersonajesConectados().get(
					paqueteComerciar.getIdEnemigo()).getItems();
			boolean loop = true;
			int i = 0;
